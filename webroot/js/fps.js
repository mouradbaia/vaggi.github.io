...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  if (typeof adlinkfly_domains !== 'undefined') {
    for (var i = 0; i < anchors.length; i++) {
      ...
      if (hostname.length > 0 && adlinkfly_domains.indexOf(hostname) > -1) {
        anchors[i].href = adlinkfly_url + 'full/?api=' + encodeURIComponent(
          adlinkfly_api_token
        ) + '&url=' + adlinkfly_base64_encode(anchors[i].href) + '&type=' + encodeURIComponent(
          advert_type
        )
      } else {
  ...
  if (typeof adlinkfly_exclude_domains !== 'undefined') {
    for (var i = 0; i < anchors.length; i++) {
      var hostname = adlinkfly_get_host_name(anchors[i].getAttribute('href'))
      if (hostname.length > 0 && adlinkfly_exclude_domains.indexOf(hostname) === -1) {
        anchors[i].href = adlinkfly_url + 'full/?api=' + encodeURIComponent(
          adlinkfly_api_token
        ) + '&url=' + adlinkfly_base64_encode(anchors[i].href) + '&type=' + encodeURIComponent(
          advert_type
